...
export async function capitalFetch<T>(
  ...
  let res: Response;
  try {
    res = await fetch(`${capitalBaseUrl()}${path}`, {
      ...init,
    ...
    if (error instanceof Error && error.name === "AbortError") {
      throw new Error(`Capital.com request timed out (45s): ${path}`);
    }
    const detail = error instanceof Error ? error.message : String(error);
    throw new Error(`Capital.com fetch failed (${path}): ${detail}`);
  } finally {
    clearTimeout(timer);
  }
...
export async function resolveCapitalEpic(symbol: string): Promise<string> {
  ...
  for (const term of searchTerms) {
    ...
    const exact =
      ...
      markets.find((m) => norm(m.epic ?? "") === norm(key)) ??
      markets.find((m) => norm(m.symbol ?? "") === norm(key)) ??
      markets.find((m) => m.instrumentName?.toUpperCase().includes(key.replace(/\//g, "")));
...
/** Fetch only bars newer than `afterTimestamp` (for forward polling). */
export async function fetchCapitalCandlesDelta(params: {
  symbol: string;
  resolution: CandleResolution;
  afterTimestamp: number;
  endTimestamp: number;
  onProgress?: (message: string) => void;
}): Promise<{ epic: string; candles: LighterCandle[] }> {
  const epic = await resolveCapitalEpic(params.symbol);
  const all = new Map<number, LighterCandle>();
  ...
  while (cursorTo > params.afterTimestamp && pages < maxPages) {
    pages++;
    params.onProgress?.(`Capital ${params.symbol} (${epic}): delta page ${pages}…`);
    ...
    for (const c of batch) {
      if (c.t > params.afterTimestamp && c.t <= params.endTimestamp) {
        all.set(c.t, c);
      }
    }

    const earliest = batch[0].t;
    if (earliest <= params.afterTimestamp) break;
  ...
  return { epic, candles: [...all.values()].sort((a, b) => a.t - b.t) };
}
